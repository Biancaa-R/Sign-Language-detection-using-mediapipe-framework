    ...
    while True:
        ...
        if results.multi_hand_landmarks is not None:
            ...
            for hand_landmarks, handedness in zip(results.multi_hand_landmarks,
                ...
                #Not using the mp.hands_connected
...
def calc_bounding_rect(image, landmarks):
    ...
    for _, landmark in enumerate(landmarks.landmark):
        ...
        # parameter indicates that the append operation should be along the first axis, effectively adding a new row to the array.
...
def calc_landmark_list(image, landmarks):
    ...
    for _, landmark in enumerate(landmarks.landmark):
        landmark_x = min(int(landmark.x * image_width), image_width - 1)
        landmark_y = min(int(landmark.y * image_height), image_height - 1)
        #Getting the landmark as a coordinate value
...
def draw_point_history(image, point_history):
    for index, point in enumerate(point_history):
        if point[0] != 0 and point[1] != 0:
            ...
            #1 + int(index / 2): The radius of the circle. The radius is dynamic and increases with the index. 
            #The int(index / 2) part suggests that the radius is increasing with the index of the point.
